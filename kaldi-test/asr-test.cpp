...
void asr_online_final_callback(void *userId, stdchar* result_text , float start_time, float end_time) {
	...
	if (result_text != L"") {
		...
		fprintf(f_res, "%s %.2f %.2f ", "talk3_8k", start_time, end_time);
		fwprintf(f_res, L"%s\n", result_text);
		fclose(f_res);
	}
...
int main()
{
	FILE *f_res = fopen("res_info.txt", "w");
	fclose(f_res);

	//初始化模型
	void *pHandle=NULL;
	const char*acModel = "../8k-model-0920/final.mdl";
	const char*wordsFile = "words.txt";
	const char*decoderGraph = "../8k-model-0920/HCLG.fst.vector";
	...
	const char * wavename = "../8k-model/talk3-8k.wav";
	FILE *fp;
	...
	int length_splice = 1600;//每次送入的数据长度，可变化。
	int i = 0;
